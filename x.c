...
{
        int len, btn, code, x = evcol(e), y = evrow(e),
                            state = e->xbutton.state; /*, button = e->xbutton.button - unused */
        char buf[40];
        static int ox, oy;
...
int ximopen(Display *dpy)
{
        ...
        if (xw.ime.xic == NULL) {
                xw.ime.xic
                    = XCreateIC(xw.ime.xim, XNInputStyle, XIMPreeditNothing | XIMStatusNothing,
...
void xinit(int cols, int rows)
{
        ...
        xw.win = XCreateWindow(
            xw.dpy, parent, xw.l, xw.t, win.w, win.h, 0, XDefaultDepth(xw.dpy, xw.scr), InputOutput,
            xw.vis, CWBackPixel | CWBorderPixel | CWBitGravity | CWEventMask | CWColormap,
            &xw.attrs);
                    ...
                    || selected(x + i, y) != selected(x + start, y) || i == (len - 1)) {
                        ...
                        for (int code_idx = 0; code_idx < shaped.count; code_idx++) {
                                ...
                                if (code_idx > 0 && idx != shaped.glyphs[code_idx - 1].cluster) {
                                        ...
                                        runewidth
                                            = win.cw
                                              * ((glyphs[start + idx].mode & ATTR_WIDE) ? 2.0f
                                                                                        : 1.0f);
                                }
                                ...
                                if (glyphs[start + code_idx].mode & ATTR_BOXDRAW) {
                                        ...
                                        if (f >= frclen) {
                                                if (!font->set)
                                                        font->set = FcFontSort(0, font->pattern, 1,
                                                                               0, &fcres);
                                                fcsets[0] = font->set;
                                                ...
                                                glyphidx
                                                    = XftCharIndex(xw.dpy, frc[frclen].font, rune);
